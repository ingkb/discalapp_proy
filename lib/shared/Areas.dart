class Areas {
  static const String suma = "Suma";
  static const String resta = "Resta";
  static const String multiplicacion = "Multiplicación";
  static const String escritura = "Escritura";
  static const String comparacion = "Comparación";
  static const String conteo = "Conteo";
  static const String rectaNumerica = "Recta numérica";
  static const String secuencia = "Secuencia";
  static const String puntos = "Puntos";
  static const String lineas = "Lineas";
  static const String menormayor = "Número Mayor";
}
